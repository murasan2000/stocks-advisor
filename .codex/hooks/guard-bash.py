...
これは `.claude/hooks/guard-bash.py` の Codex 版。判定ロジックは同一で、
ブロック時に返す JSON の形だけが違う（`deny()` のコメント参照）。

役割分担の原則:
  - permissions で表現できるものは、そちらに任せる。
  - このフックは「パターンでは書きにくいもの」だけを担当する。具体的には
    フラグの書き方が複数あるもの（force push、rm の再帰フラグ）と、
    パスの種類（絶対パスか、プロジェクト内の相対パスか）で可否が変わるもの。

なぜ Bash だけを見るのか:
  Codex の PreToolUse は現状 Bash/シェル実行が主な対象であることに加え、
  シェルのリダイレクトや sed -i はファイル編集用の権限ルールの検査対象外で
  素通りしてしまうため。

ブロック方法:
  exit 0 + `{"decision": "block", "reason": ...}` を返す（理由がエージェントに伝わる）。
  判断しない場合は exit 0 で何も出力せず、通常の確認フローに委ねる。
...
# シェルの区切り文字。エージェントの Bash ルール判定と同様、区切られた各コマンドを
...
RULES: list[tuple[object, str]] = [
    (
        _touches_env_file,
        ".env 系ファイルは秘密情報を含むため、エージェントからの読み書きを禁止しています。"
...
def deny(reason: str) -> None:
    """Codex の PreToolUse 契約でブロックする。

    Claude Code 版（`.claude/hooks/guard-bash.py`）とはここだけ実装が違う。
    Claude Code は `hookSpecificOutput.permissionDecision="deny"` を見るが、
    Codex は `decision="block"` / `reason` を見るため、同じ JSON を返しても
    Codex 側では何もブロックされない（素通りする）。
    """
    json.dump(
