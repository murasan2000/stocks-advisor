...
Codex（ChatGPT デスクトップアプリ / Codex CLI）向け。

> **このファイルの位置づけ（重要）**
>
> - **正は [CLAUDE.md](CLAUDE.md)**。このファイルはそれを Codex 向けに置き換えた
>   コピーで、放っておくと必ずズレる。実際、最初に生成された版は生成元の
>   CLAUDE.md がその後更新されたことで、コミットメッセージ規約が古いまま
>   残っていた。**規約を変えるときは CLAUDE.md を直し、このファイルへ反映する。**
> - **Codex での動作は未検証**。この一式は ChatGPT デスクトップアプリが
>   リポジトリを認識して自動生成したもので、実際に Codex で通して動かしたことは
>   まだない。
> - 下の「ガードレール」節のうち **permissions（allow / ask / deny）は
>   Claude Code 固有の仕組み**で、Codex 側には対応する設定が無い。Codex で効くのは
>   `.codex/hooks.json` に登録したフックだけ。
...
.codex/            Codex のフック・サブエージェント定義（詳細は「ガードレール」節）
.agents/skills/    スキル（.claude/skills/ の写し）
```
...
実装はローカルの Codex CLI が担い、検証は GitHub Actions が Codex 抜きで
再現する、という役割分担を取る。

1. **Issue 作成**: `gh issue create`、または GitHub 上で直接。
2. **実装**: ローカル CLI が Issue を読み、この AGENTS.md の規約に従って実装する。
...
- **ブランチ運用・commit/push・コミットメッセージ規約は `git-workflow` skill に従う**
  （`.agents/skills/git-workflow/SKILL.md`）。要点: 作業前に必ず `claude/feature/<topic>`
  ...
  準拠、scope はプロダクトコードとエージェント運用ルール自身を区別する。
  詳細は git-workflow skill 参照）。
- **CI はローカル検証を省く理由にはしない**。CI の役割は「Codex が自分の変更を
...
## ガードレール（.codex/ と .agents/）

Codex で実際に効くのは `.codex/hooks.json` に登録した 2 つのフックだけ。
以降の permissions（allow / ask / deny）の記述は Claude Code 側の仕組みの説明で、
Codex には対応物が無い（`.claude/settings.json` を参照）。意図の記録として残している。

- **SessionStart フック（`.codex/hooks/session-start.sh`）**: `api/.venv` と
  `web/node_modules` が無いときだけ依存をインストールする（冪等）。
  `.devcontainer` の `postCreateCommand` は `api/` の `uv sync` しか行わないため、
  これが無いと `web/` の `npm run lint` / `npm run build` が実行できない。
  Claude Code 版は `$CLAUDE_CODE_REMOTE` でリモート環境だけに絞っているが、
  Codex にはその区別が無いので「入っていなければ入れる」判定にしてある。
- **PreToolUse フック（`.codex/hooks/guard-bash.py`）**: Bash 経由の破壊的操作・
  秘密情報アクセスをブロックする。判定ロジックは Claude Code 版と同一だが、
  **ブロック時に返す JSON の形が違う**（Codex は `{"decision": "block", "reason": ...}`、
  Claude Code は `hookSpecificOutput.permissionDecision`）。ここを取り違えると、
  フックは動いているのに何もブロックしない状態になる。

- **allow**: ファイル編集（`Edit` / `Write`、全パス対象）と、テスト・lint・ビルド・
  git / gh の大半（`git push`、`gh pr create` 等の外向き操作を含む）。実装は
  ローカルの Codex / gh 認証で完結し、最終的に PR で人間がレビューするため、
  この粒度の操作は都度の確認を求めない。
- **ask**: 依存の増減（`uv add` 等。インストールスクリプト経由でコードが実行され
  得るため）、および CI 定義・ガードレール自身の編集（`Edit(/.github/workflows/**)` /
  `Edit(/.codex/**)`。Codex が自分自身の制限を気づかれずに緩められないようにする
  ため）。これらはパス指定の `ask` ルールが `allow` の `Edit`/`Write` より先に評価
  されるため、他のファイル編集が確認不要でもこの2つだけは確認が残る。
  それ以外の「本当に危ない操作」（未知のサイトへのデータ送信・未知のサイトからの
  取得など）は個別の allow ルールを置いていないため、既定の確認フローに委ねている
  （`WebSearch` は既定で確認不要、`WebFetch` は初見のドメインごとに確認が入る）。
- **deny**: `.env` 系の読み書き、ロックファイルの直接編集、`gh secret`、force push。
- パスを対象にする権限ルールは **`Edit(...)` / `Read(...)` のみが参照される**。
  `Write(...)` で書いてもパス指定ルールは無視され、起動時に警告が出るので使わない
  こと（パス指定なしの bare `Write` ルール自体は有効で、上記の allow で使っている）。
