...
- **allow**: ファイル編集（`Edit` / `Write`、全パス対象）と、テスト・lint・ビルド・
  ...
  ローカルの Claude Code / gh 認証で完結し、最終的に PR で人間がレビューするため、
  この粒度の操作は都度の確認を求めない。
- **ask**: 依存の増減（`uv add` 等。インストールスクリプト経由でコードが実行され
  ...
  `Edit(/.claude/hooks/**)`。Claude が自分自身の制限を気づかれずに緩められないように
  するため）。これらはパス指定の `ask` ルールが `allow` の `Edit`/`Write` より先に
  評価されるため、他のファイル編集が確認不要でもこれらだけは確認が残る。
  **`Edit(/.claude/**)` のような `.claude/` 配下を丸ごと対象にする書き方はしない**。
  並列サブエージェント用の git worktree が `.claude/worktrees/` 配下に作られるため
  （`.gitignore` 参照）、丸ごと指定すると worktree 内で編集する実装ファイル
  （`api/tests/...` や `web/src/...` 等）まで `.claude/**` にマッチしてしまい、
  ガードレールと無関係な実装編集のたびに確認が出る（クラウド版の並列実行で特に
  頻発する）。保護したいのは「ガードレール自身」＝ `settings.json` と
  `hooks/` だけなので、対象はそのファイル・ディレクトリだけをピンポイントで指定する。
